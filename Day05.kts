...
fun lt(cursor: Int, mode: String, program: MutableList<Int>): Int {
  val x = getValue(program.get(cursor + 1), mode.get(1), program)
  val y = getValue(program.get(cursor + 2), mode.get(0), program)
  val addr = program.get(cursor + 3)
  println("LSS: $x < $y -> $addr")
  return if ( x < y ) addr else cursor + 4
}

fun eq(cursor: Int, mode: String, program: MutableList<Int>): Int {
  val x = getValue(program.get(cursor + 1), mode.get(1), program)
  val y = getValue(program.get(cursor + 2), mode.get(0), program)
  val addr = program.get(cursor + 3)
...
fun computer(program: MutableList<Int>, cursor: Int = 0, input: Int = 5) {
  ...
  when (opCode) {
    ...
    7 -> return computer(program, lt(cursor, mode, program))
    8 -> return computer(program, eq(cursor, mode, program))
...
// computer(listOf(3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99))
